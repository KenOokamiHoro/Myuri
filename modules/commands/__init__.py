...
def dispatch(bot, nick, message, target, send):
    if message[:1] not in ["'", '"']:
        return
    try:
        bot.is_refuse(nick,target)
    except irc.client.RefuseException:
        print("refused")
        refuse_text = random.choice(bot.refuse_templates)
        bot.refuse(target,refuse_text)
        message = "'refuse "+refuse_text
